defmodule Pretty.Grid.Placement do
  ...
  defp place_items_direction_row(
    ...
    # we have to check if there are any more rows 
    more_rows? =
      Enum.any?(0..(column_count - 1), fn col ->
        MapSet.member?(occupied_cells, {row + 1, col})
      end)
  ...
  defp place_items_direction_row(
    ...
    overlap? = MapSet.member?(occupied_cells, {row, col})
  ...
  defp place_items_direction_row(
    ...
    if col >= column_count do
      ...
      # if the item does not fit the remaining space in the row, add empty item and try again.
      if item.column_span > column_count - col do
        place_items_direction_row(
          [Item.empty(empty_id) | items],
          placed_items,
          row,
          col,
          occupied_cells,
  ...
  defp place_items_direction_column(
    ...
    # we have to check if there are any more columns
    more_columns? =
      Enum.any?(0..(row_count - 1), fn row -> MapSet.member?(occupied_cells, {row, col + 1}) end)
  ...
  defp place_items_direction_column(
       ...
       when row < row_count do
    # fill the rest of the column with empty items
    overlap? = MapSet.member?(occupied_cells, {row, col})
  ...
  defp place_items_direction_column(
    ...
    if row >= row_count do
      ...
      # if the item does not fit the remaining space in the column, add empty item and try again.
